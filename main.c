...
void remv(struct cart* h);
void displayLL(struct cart* h);
void process(struct cart* h);
    ...
    while (y=!0){
        ...
        if (x==2){
        	...
        	for(a=0;a<5;a++)
            {
                ...
                {
                    printf("\n\n isbn no.      Name        Author      unit price (LKR)\n\n");
                    printf("\n  %d\t      %s\t  %s\t   %d",items[a].isbn,items[a].title,items[a].author,items[a].price);
                    b=0;
                }
...
void displayLL(struct cart* h)
{
	...
	while(loop!=NULL){
            printf("\n  %d\t", (*loop).isbn); printf("      %s\t\t",(*loop).title); printf("  %d\t\t", (*loop).price); printf("  %d\t\t", (*loop).quantity); printf(" %d", (*loop).total);
